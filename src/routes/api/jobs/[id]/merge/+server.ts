...
// POST /api/jobs/:id/merge - Merge all scraped data for a job into combined_data
export const POST: RequestHandler = async ({ params }) => {
	const supabase = getSupabase();

	// Get all completed scraped data for this job
	...
	if (!scrapedData || scrapedData.length === 0) {
		return json({ error: 'No completed scrape data found for this job' }, { status: 404 });
	}

	// Extract the data objects
	const dataObjects = scrapedData
		.map(d => d.extracted_data as Record<string, unknown>)
		.filter(Boolean);

	// Merge the data
	const combinedData = mergeExtractedData(dataObjects);

	// Update the job with combined data
